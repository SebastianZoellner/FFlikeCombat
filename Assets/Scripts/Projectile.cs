...
public class Projectile : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        /*
            if (other.GetComponent<CharacterHealth>() == targetHealth)
            {
                if(!hasHit)
                {
                    targetHealth.GetComponentInChildren<DamageTextSpawner>().CallMiss();
                    Destroy(gameObject);
                    return;
                }
                if (!targetHealth.GetIsDead())
                {
                    targetHealth.TakeDamage(shooter, damage,attackPower.GetStunTime());
                    Impact(hitPosition);
                }
            }
            else
