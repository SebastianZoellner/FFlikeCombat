...
public class BoostCombatStatus : BaseStatus
{
    int turnCounter;
    GameObject activeVFX;

    public BoostCombatStatus(StatusManager statusManager, float intensity, float damageModifier, int duration, GameObject statusVFX) : base(statusManager, intensity, damageModifier, duration, statusVFX)
    {
    }
    ...
    if (activeVFX)
    {
        Debug.Log("Destroying BoostCombat VFX");
